...
export default function ChatWindow({
  ...
  useEffect(() => {
    ...
    chatAPI
      ...
      .catch((err: any) => {
        ...
        setError(
          typeof data === "string" ? data : data?.error || t("chat.load_error"),
        );
      })
      .finally(() => {
  ...
  const submit = async () => {
    ...
    if (editing) {
      ...
      try {
        ...
        setMessages((prev) =>
          prev.map((m) => (m.id === editing.id ? { ...m, ...res.data } : m)),
        );
        setEditing(null);
        setInput("");
      } catch (err: any) {
        ...
        setError(
          typeof data === "string"
            ? data
            : data?.error || t("auth.error_occurred"),
        );
      } finally {
        setBusy(false);
      }
  ...
  const isMine = (message: MessageData) => {
    const senderId =
      message.senderId ?? message.sender ?? message.sender_details?.id;
    return !!myId && senderId != null && String(senderId) === myId;
  };
        ...
        <div className="min-w-0">
          ...
          >
            <span
              ...
              style={{
                background: connected ? "var(--success)" : "var(--text-light)",
              }}
            />
                ...
                <div className="relative max-w-[78%]">
                  ...
                  <div
                    className={`px-3.5 py-2.5 rounded-2xl text-sm leading-relaxed cursor-pointer ${
                      mine
                        ? "rounded-br-md ml-auto bg-[var(--primary)] text-white"
                        : "rounded-bl-md mr-auto bg-[var(--surface)] text-[var(--text)]"
                    }`}
                    style={
                      mine ? {} : { boxShadow: "0 1px 2px rgba(0,0,0,0.06)" }
                    }
                    onClick={(e) => {
                      e.stopPropagation();
